...
function line(ctx, width, colour, iteration, angle) {
    ...
    ctx.beginPath();
    ctx.moveTo((ctx.canvas.width / 2) + (radius1 * Math.cos(angle)), (ctx.canvas.height / 2) + (radius1 * Math.sin(angle)));
    ctx.lineTo((ctx.canvas.width / 2) + (radius2 * Math.cos(angle)), (ctx.canvas.height / 2) + (radius2 * Math.sin(angle)));
    ctx.strokeStyle = colour;
    ctx.stroke();
}
